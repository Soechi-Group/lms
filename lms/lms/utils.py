...
def get_lessons(course, chapter=None, get_details=True, progress=False):
    ...
    for chapter in get_chapters(course):
        if get_details:
            ...
            lesson_count += frappe.db.count("Lesson Reference",
                                            {"parent": chapter.name})
...
def get_reviews(course):
    ...
    out_of_ratings = frappe.db.get_all(
        "DocField", {"parent": "LMS Course Review",
                     "fieldtype": "Rating"}, ["options"]
    )
...
def get_lesson_index(lesson_name):
    ...
    lesson = frappe.db.get_value("Lesson Reference", {"lesson": lesson_name}, [
                                 "idx", "parent"], as_dict=True)
    if not lesson:
        return "1-1"
...
def render_html(lesson):
    ...
    if lesson.question:
        assignment = "{{ Assignment('" + lesson.question + \
            "-" + lesson.file_type + "') }}"
...
def is_cohort_staff(course, user_email):
    ...
    mentor = {"doctype": "Cohort Mentor",
              "course": course, "email": user_email}
...
def get_mentors(course):
    ...
    course_mentors = []
    mentors = frappe.get_all("LMS Course Mentor Mapping", {
...
def get_course_progress(course, member=None):
    ...
    completed_lessons = frappe.db.count(
        "LMS Course Progress",
        {"course": course, "member": member or frappe.session.user,
         "status": "Complete"},
    )
...
def get_initial_members(course):
    members = frappe.get_all(
        "LMS Enrollment", {"course": course}, ["member"], limit=3)
    ...
    for check in checks:
        if frappe.db.get_single_value("LMS Settings", check):
            page = frappe.db.get_single_value(
                "LMS Settings", mapper[check].get("page_name"))
            route = frappe.db.get_value("Web Page", page, "route")
            links.append("<a href='/" + route + "'>" +
                         mapper[check].get("title") + "</a>")
...
def create_notification_log(doc, topic):
    ...
    if topic.reference_doctype == "Course Lesson":
        course = frappe.db.get_value(
            "Course Lesson", topic.reference_docname, "course")
        ...
        link = get_lesson_url(
            course, get_lesson_index(topic.reference_docname))

    else:
        batch_title = frappe.db.get_value(
            "LMS Batch", topic.reference_docname, "title")
        subject = _("New comment in batch {0}").format(batch_title)
        link = f"/batches/{topic.reference_docname}"
        moderators = frappe.get_all(
            "Has Role", {"role": "Moderator"}, pluck="parent")
        users += moderators
...
def get_all_memberships(member):
    return frappe.get_all(
        "LMS Enrollment",
        {"member": member},
        ["name", "course", "batch_old",
         "current_lesson", "member_type", "progress"],
    )


def get_filtered_membership(course, memberships):
    current_membership = list(
        filter(lambda x: x.course == course, memberships))
...
def get_upcoming_evals(student, courses, batch=None):
    ...
    for evals in upcoming_evals:
        evals.course_title = frappe.db.get_value(
            "LMS Course", evals.course, "title")
        evals.evaluator_name = frappe.db.get_value(
            "User", evals.evaluator, "full_name")
...
def get_course_details(course):
    ...
    course_details.instructors = get_instructors(
        "LMS Course", course_details.name)
...
def get_course_outline(course, progress=False):
    ...
    outline = []
    chapters = frappe.get_all("Chapter Reference", {"parent": course}, [
                              "chapter", "idx"], order_by="idx")
    for chapter in chapters:
        chapter_details = frappe.db.get_value(
            "Course Chapter",
            chapter.chapter,
            ["name", "title", "is_scorm_package",
             "launch_file", "scorm_package"],
            as_dict=True,
        )
...
def get_neighbour_lesson(course, chapter, lesson):
    ...
    for chapter in chapters:
        lessons = frappe.get_all("Lesson Reference", {
                                 "parent": chapter.chapter}, pluck="idx")
...
def get_batch_details(batch):
    batch_students = frappe.get_all("LMS Batch Enrollment", {
                                    "batch": batch}, pluck="member")
    if (
    ...
    if batch_details.paid_batch and batch_details.start_date >= getdate():
        ...
        batch_details.price = fmt_money(
            batch_details.amount, 0, batch_details.currency)

    if batch_details.seat_count:
        batch_details.seats_left = batch_details.seat_count - \
            len(batch_details.students)
...
def get_question_details(question):
    ...
    question_details = frappe.db.get_value(
        "LMS Question", question, fields, as_dict=1)
...
def get_batch_courses(batch):
    ...
    course_list = frappe.get_all(
        "Batch Course", {"parent": batch}, ["name", "course"])
...
def get_quiz_details(assessment, member):
    ...
    submission_name = existing_submission[0].name if len(
        existing_submission) else "new-submission"
...
def get_batch_students(batch):
    ...
    batch_courses = frappe.get_all(
        "Batch Course", {"parent": batch}, ["course", "title"])
    assessments = frappe.get_all(
        "LMS Assessment",
    ...
    for student in students_list:
        ...
        for course in batch_courses:
            progress = frappe.db.get_value(
                "LMS Enrollment", {"course": course.course,
                                   "member": student.member}, "progress"
            )
        ...
        if len(batch_courses) + len(assessments):
            detail.progress = flt(
                ((courses_completed + assessments_completed) /
                 (len(batch_courses) + len(assessments)) * 100),
                2,
            )
        else:
            detail.progress = 0
...
def has_submitted_assessment(assessment, assessment_type, member=None):
    ...
    if attempt:
        fields.append("name")
        attempt_details = frappe.db.get_value(
            doctype, filters, fields, as_dict=1)
...
def get_order_summary(doctype, docname, country=None):
    ...
    details.original_amount_formatted = fmt_money(
        details.amount, 0, details.currency)

    if details.currency == "INR":
        details.amount, details.gst_applied = apply_gst(
            details.amount, country)
        details.gst_amount_formatted = fmt_money(
            details.gst_applied, 0, details.currency)

    details.total_amount_formatted = fmt_money(
        details.amount, 0, details.currency)
    return details
...
def publish_notifications(doc, method):
    frappe.publish_realtime("publish_lms_notifications",
                            user=doc.for_user, after_commit=True)
...
def update_payment_record(doctype, docname):
    ...
    if len(request):
        data = frappe.db.get_value(
            "Integration Request", request[0].name, "data")
...
def enroll_in_course(course, payment_name):
    if not frappe.db.exists("LMS Enrollment", {"member": frappe.session.user, "course": course}):
        ...
        payment = frappe.db.get_value("LMS Payment", payment_name, [
...
def enroll_in_batch(batch, payment_name=None):
    if not frappe.db.exists("LMS Batch Enrollment", {"batch": batch, "member": frappe.session.user}):
        ...
        if batch_doc.seat_count and students >= batch_doc.seat_count:
            frappe.throw(
                _("The batch is full. Please contact the Administrator."))
...
def get_programs():
    if has_course_moderator_role() or has_course_instructor_role() or has_course_evaluator_role():
        ...
        programs = frappe.get_all(
            "LMS Program Member", {"member": frappe.session.user}, [
    ...
    for program in programs:
        ...
        program.members = frappe.db.count(
...
def enroll_in_program_course(program, course):
    ...
    if enrollment:
        enrollment = frappe.db.get_value("LMS Enrollment", enrollment, [
                                         "name", "current_lesson"], as_dict=1)
...
def get_batch_card_details(batches):
    for batch in batches:
        ...
        students_count = frappe.db.count(
            "LMS Batch Enrollment", {"batch": batch.name})
